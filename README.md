...
* **convert-rdf-to-turtle** - converts rdf file/files to turtle
  * parameters:
...
# Format of RDF artefacts
The following RDF files are generated by model2owl for each supported RDF artefact:
1. `*.owl`: RDF/XML serialization (OWL API flavor).
2. `*-ttl.owl`: Turtle serialization (OWL API flavor); optional, produced by `convert-ontology-rdfxml-to-turtle` *Make* recipe.
3. `*.rdf`: RDF/XML serialization.
4. `*.ttl`: Turtle serialization; optional, produced by `convert-rdf-to-turtle` *Make* recipe.
