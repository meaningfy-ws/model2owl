...
# suffix for the optional consolidated OWL-full artefact (.owl/.rdf/.ttl)
FULL_FILE_SUFFIX?=_full
...
######################################################################################
install:  get-saxon get-rdflib get-robot get-widoco get-jena-cli-tools get-jinja get-jq
...
# Optional consolidated OWL-full artefact: a smart (ontology-valid) merge of the OWL
# core and OWL restrictions artefacts into a single self-contained ontology - an
# alternative form of the ontology, the two OWL artefacts consolidated for the user's
# convenience. Gated by the `generateOWLFull` config flag (disabled by default).
# It reuses existing core/restrictions outputs when present, and generates them when
# absent. Ownership rule: it removes ONLY the core/restrictions intermediates it
# generated itself in this run - pre-existing (caller-owned) files are left untouched.
# The heavy lifting lives in scripts/owl-full/generate-owl-full.sh.
#
# make owl-full [XMI_INPUT_FILE_PATH=/path/to/cm.xmi] [OUTPUT_FOLDER_PATH=/output/dir]
#	[NAMESPACES_USER_XML_FILE_PATH=...] [IMPORTS_XML_FILE_PATH=...]
owl-full:
	@enabled=$$(java -jar ${SAXON} -it:main -xsl:${MODEL2OWL_FOLDER}/src/xml/emit-config-value.xsl key=generateOWLFull 2>/dev/null); \
	if [ "$$enabled" != "true" ]; then \
		echo "OWL-full disabled (generateOWLFull=false in the active config); skipping 'owl-full'."; \
		exit 0; \
	fi; \
	$(MAKE) gen-enriched-ns-file; \
	core_owl="${OUTPUT_FOLDER_PATH}/${XMI_INPUT_FILENAME_WITHOUT_EXTENSION}${CORE_OWL_FILE_SUFFIX}.owl"; \
	core_rdf="${OUTPUT_FOLDER_PATH}/${XMI_INPUT_FILENAME_WITHOUT_EXTENSION}${CORE_RDF_FILE_SUFFIX}.rdf"; \
	restr_owl="${OUTPUT_FOLDER_PATH}/${XMI_INPUT_FILENAME_WITHOUT_EXTENSION}${RESTRICTION_OWL_FILE_SUFFIX}.owl"; \
	restr_rdf="${OUTPUT_FOLDER_PATH}/${XMI_INPUT_FILENAME_WITHOUT_EXTENSION}${RESTRICTION_RDF_FILE_SUFFIX}.rdf"; \
	created=""; \
	if [ ! -f "$$core_owl" ]; then $(MAKE) owl-core; created="$$created $$core_owl $$core_rdf"; \
	else echo "[owl-full] reusing existing core artefact (not generated by owl-full; will be kept)"; fi; \
	if [ ! -f "$$restr_owl" ]; then $(MAKE) owl-restrictions; created="$$created $$restr_owl $$restr_rdf"; \
	else echo "[owl-full] reusing existing restrictions artefact (not generated by owl-full; will be kept)"; fi; \
	full_uri=$$(java -jar ${SAXON} -it:main -xsl:${MODEL2OWL_FOLDER}/src/xml/emit-config-value.xsl key=fullArtefactURI 2>/dev/null); \
	MODEL2OWL_FOLDER="${MODEL2OWL_FOLDER}" SAXON="${SAXON}" ROBOT="robot/robot.jar" \
		XMI_INPUT_FILE_PATH="${XMI_INPUT_FILE_PATH}" \
		ENRICHED_NAMESPACES_XML_PATH="${ENRICHED_NAMESPACES_XML_PATH}" \
		IMPORTS_XML_FILE_PATH="${IMPORTS_XML_FILE_PATH}" \
		SAXON_METADATA_PARAM='${SAXON_METADATA_PARAM}' \
		FULL_URI="$$full_uri" \
		CORE_OWL="$$core_owl" RESTR_OWL="$$restr_owl" \
		CATALOG="${RESTR_CATALOG_PATH}" \
		FULL_OWL="${OUTPUT_FOLDER_PATH}/${XMI_INPUT_FILENAME_WITHOUT_EXTENSION}${FULL_FILE_SUFFIX}.owl" \
		FULL_RDF="${OUTPUT_FOLDER_PATH}/${XMI_INPUT_FILENAME_WITHOUT_EXTENSION}${FULL_FILE_SUFFIX}.rdf" \
		FULL_TTL="${OUTPUT_FOLDER_PATH}/${XMI_INPUT_FILENAME_WITHOUT_EXTENSION}${FULL_FILE_SUFFIX}.ttl" \
		HEADER_RDF="${OUTPUT_FOLDER_PATH}/${XMI_INPUT_FILENAME_WITHOUT_EXTENSION}${FULL_FILE_SUFFIX}-header.tmp.rdf" \
		RDF_XML_MIME_TYPE=${RDF_XML_MIME_TYPE} TURTLE_MIME_TYPE=${TURTLE_MIME_TYPE} \
		CLEANUP_FILES="$$created" \
		bash ${MODEL2OWL_FOLDER}/scripts/owl-full/generate-owl-full.sh
...
# Structural validation using rdflib (rdfpipe): parses the file and fails on
# malformed RDF. Unlike validate-rdf-file (Jena RIOT) it only needs the rdflib
# venv, which is already required wherever the RDF serializations are produced -
# so it works in lightweight contexts (e.g. the boilerplate transform job) that
# do not install the Jena CLI tools.
# make validate-rdf-file-rdflib FILE_TO_VALIDATE_PATH=./output/eFulfilment.rdf
validate-rdf-file-rdflib:
	@source model2owl-venv/bin/activate; \
	rdfpipe -i ${RDF_XML_MIME_TYPE} -o ${RDF_XML_MIME_TYPE} $(FILE_TO_VALIDATE_PATH) > /dev/null
